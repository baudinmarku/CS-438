...
function createCircleGeometry(radius, segments) {
    ...
    for(var i = 0; i <= segments; i++) {
        // Push a new triangle for every segment
        points.push(vec2(radius * Math.cos(step * i), radius * Math.sin(step * i)));
...
function main() {
    ...
    function render() {
        ...
        gl.enableVertexAttribArray(colorAttribLocation);
        // Draws the data in the shape of a triangle fan
        gl.drawArrays(gl.TRIANGLE_FAN, 0, geometry.length / 5);
        // Disable the color attribute
        gl.disableVertexAttribArray(colorAttribLocation);
        // Draws the line around the "circle"
