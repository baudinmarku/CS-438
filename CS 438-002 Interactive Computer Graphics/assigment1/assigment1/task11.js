...
function createTriangleGasketGeometry(recursions, lambda) {
    ...
    function divideTriangle(a, b, c, ca, cb, cc, count) {
        ...
        if (count == 0) {
            ...
            // Left triangle
            divideTriangle(a, mix(a, b, lambda), mix(a, c ,lambda), ca, mix(ca, cb, lambda), mix(ca, cc, lambda), count - 1);
            // Top triangle
            divideTriangle(mix(a, b, lambda), b, mix(b, c ,lambda), mix(ca, cb, lambda), cb, mix(cb, cc, lambda), count - 1);
            // Right triangle
